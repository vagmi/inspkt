...
describe("inspections repo", () => {
  ...
  it("lists per org with equipment, type, facility, client, and form names", async () => {
    const db = testDb();
    const repo = createInspectionsRepo(db);
    ...
    expect(list[0].equipmentName).toBe("Unit A-1");
    expect(list[0].typeName).toBe("Rooftop HVAC");
    expect(list[0].facilityName).toBe("First Facility");
    expect(list[0].clientName).toBe("Acme Properties");
